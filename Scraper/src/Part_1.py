...
soup,page = get_page("https://movie.naver.com/movie/bi/mi/pointWriteFormList.naver?code=175092&type=after&isActualPointWriteExecute=false&isMileageSubscriptionAlready=false&isMileageSubscriptionReject=false")
text = soup.find_all('span',id=re.compile('_filtered_ment_'))
score = soup.find_all('div','star_score')

reviews=[]
for a,b in zip(text,score):
    review_text = str(a.get_text().strip())
    review_star = int(b.get_text().strip())
    all = ('review_text :', a.get_text().strip(),'review_star:', b.get_text().strip())
    reviews.append(all)
...
def get_avg_stars(reviews):
    ...
    for i in range(0,len(reviews)):
        score_list.append(reviews[i]['review_star'])
...
def get_reviews(movie_code, page_num=1):
    ...
    data = soup.find_all('td','title')
    review_list = []
    for i in data:
        review = {'review_text':str(i.get_text("_",strip=True).split('_')[3]),
                   'review_star': int(i.get_text("_",strip=True).split('_')[2])}
...
def scrape_by_review_num(movie_title, review_num):
    ...
    list=[]
    page = (review_num//10) + 1
    num = review_num%10
    a = get_reviews(movie_code,1)
    for i in range(2,page+1):
        a = a+ get_reviews(movie_code,i)
    reviews = []
    for b in a:
        reviews.append(b['review_text'])
    reviews = reviews[0:review_num]
    return reviews
